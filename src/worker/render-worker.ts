...
let _offscreenCanvas: OffscreenCanvas;
let _canvasContext: OffscreenCanvasRenderingContext2D;
let _canvasImageData: ImageData;
let _tfWorld: TfWorld;
let _samplesRendered = 0.0;
let _totalSamples = 0;
...
onmessage = (ev: MessageEvent) => {
  switch (ev.data.msg) {
    ...
    case "render":
      render(ev.data.world);
      break;

    default:
      console.error("RenderWorker: unknown message:", ev.data);
      break;
  }
};

function init(canvas: OffscreenCanvas): void {
  console.log("RenderWorker initialized");
  _offscreenCanvas = canvas;
  _canvasContext = _offscreenCanvas.getContext("2d")!;
  _canvasImageData = _canvasContext.createImageData(
    _offscreenCanvas.width,
    _offscreenCanvas.height,
  );
}

async function render(tfWorld: TfWorld): Promise<void> {
  console.log("RenderWorker render");
  _tfWorld = tfWorld;
  _samplesRendered = 0.0;

  const world = createWorldFromTf(_tfWorld);
  ...
  const renderContext = createRenderContext(_canvasImageData, camera);
  _totalSamples =
    renderContext.width * renderContext.height * renderContext.samplesPerPixel;
  const pixelGen = renderPixelInc(renderContext, world);
  const perf = performance;
  let lastTs = perf.now();
  let lastPost = perf.now();
  ...
  while (!pixelData.done) {
    ...
    while (!pixelData.done && lastTs + 1000.0 / 30 >= perf.now()) {
      const pos = pixelData.value.position;
      writeImageDataColor(_canvasImageData, pos, pixelData.value.color);
      _samplesRendered += 1.0;
      pixelData = pixelGen.next();

      if (perf.now() > lastPost + 1000.0 / 120) {
        postMessage({
          msg: "render-update",
          progress: _samplesRendered / _totalSamples,
        });
        lastPost = perf.now();
      }
    }

    _canvasContext.putImageData(_canvasImageData, 0, 0);
